...
def apply_jnd(imgs: torch.Tensor, imgs_w: torch.Tensor, hmaps: torch.Tensor, mode: str) -> torch.Tensor:
    """ 
...
class JND(nn.Module):
    ...
    def __init__(self,
        ...
        # setup some params for applying jnd
    ...
    def forward(self, imgs: torch.Tensor, imgs_w: torch.Tensor) -> torch.Tensor:
        ...
        imgs_w = apply_jnd(imgs, imgs_w, hmaps, self.apply_mode)
        return imgs_w


class NoLumJND(nn.Module):
    """ https://ieeexplore.ieee.org/document/7885108 """
    ...
    def heatmaps(
        ...
        if self.in_channels == 1:
            imgs = rgbs[0] * imgs[...,0:1,:,:] + rgbs[1] * imgs[...,1:2,:,:] + rgbs[2] * imgs[...,2:3,:,:]  # luminance: b 1 h w
        cm = self.jnd_cm(imgs)
        hmaps = torch.clamp_min(cm, 0)   # b 1or3 h w
        if self.out_channels == 3 and self.in_channels == 1:
            # rgbs = (1-rgbs).unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
            hmaps = hmaps.repeat(1, 3, 1, 1)  # b 3 h w
            if self.blue:
...
def build_jnd(
        ...
        attenuation_cfg: dict
) -> JND | NoLumJND | JNDSimplified | VarianceBasedJND | None:
    """
    Build a JND (Just Noticeable Difference) attenuation model.
    Args:
        attenuation_name (str): Name of the attenuation model (e.g., "jnd", "nolum", "simplified_jnd", "simplified", "variance")
        attenuation_cfg (dict): Configuration for the attenuation
    """
    attenuation = None
    if attenuation_name is None or attenuation_name.lower() == "none":
        return attenuation
    
    if attenuation_name.lower().startswith("jnd"):
        attenuation = JND(**attenuation_cfg[attenuation_name])
    elif attenuation_name.lower().startswith("nolum"):
        attenuation = NoLumJND(**attenuation_cfg[attenuation_name])
    elif attenuation_name.lower().startswith("simplified"):
        attenuation = JNDSimplified(**attenuation_cfg[attenuation_name])
    elif attenuation_name.lower().startswith("variance"):
        attenuation = VarianceBasedJND(**attenuation_cfg[attenuation_name])
