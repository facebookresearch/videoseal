...
## Exporting your own model to TorchScript

If you have a trained VideoSeal model, you can convert it to TorchScript format:
...
wam.blender.scaling_w = 0.2  # Adjust watermark strength
wam.eval()
wam.to(device)
...
model_jit = WamJIT(
    ...
    wam.detector, 
    wam.attenuation, 
    scaling_w=0.2,  # Watermark strength
    scaling_i=1.0,   # Image strength
    do_attenuation=True,
...
model_jit.save("ckpts/custom_model.jit")
print(f"Size of the model: {os.path.getsize('ckpts/custom_model.jit') / 1e6:.2f} MB")
```
