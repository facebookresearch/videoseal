...
parser.add_argument('--input_file', type=str, required=True, help='Path to the input mp4 file')
parser.add_argument('--output_file', type=str, required=False, default="watermarked.mp4", help='Path to save the watermarked mp4 file')
...
if not args.detect:
    ...
    if not args.video_only:
        ...
        with torch.no_grad():
            audio_msg = torch.randint(0, 2, (audio_16k_mono_batched.shape[0], audio_model.msg_processor.nbits), device=audio_16k_mono_batched.device)
            watermark = audio_model.get_watermark(audio_16k_mono_batched, 16000, message=audio_msg)
    ...
    if not args.video_only:
        ...
        with torch.no_grad():
            result, message = detector.detect_watermark(torchaudio.transforms.Resample(sample_rate, 16000)(audio), 16000)
